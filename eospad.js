        ...
        <div class="main-container" id="mainContainer">
            ...
            <div class="toolbar-container">
                <div class="toolbar-group">
                    <button class="toolbar-button" id="historyBtn" title="Version History"><svg viewBox="0 0 24 24">
  <path d="M6 3h9l5 5v13a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2z"></path>
  <path d="M14 3v5h5"></path>
  <path d="M12 11a4 4 0 1 0 4 4"></path>
  <path d="M12 13v2h2"></path>
</svg></button>
                ...
                <div class="toolbar-group">
                    ...
                    <label for="foreColorPicker" class="toolbar-label" title="Text Color">
                        ...
                        <input type="color" id="foreColorPicker" aria-label="Text color picker">
                    </label>
                    <label for="backColorPicker" class="toolbar-label" title="Highlight Color">
                        ...
                        <input type="color" id="backColorPicker" value="#fab387" aria-label="Highlight color picker">
                    </label>
                    <div class="divider"></div>
        ...
        <div id="modalOverlay" class="modal-overlay"> <div class="modal" id="genericModal"> <h3 id="modalTitle">Modal Title</h3> <p id="modalText"></p> <input type="text" id="modalInput" class="modal-input" placeholder="Enter value here..."> <div class="modal-buttons"> <button id="modalCancel" class="modal-button secondary">Cancel</button> <button id="modalConfirm" class="modal-button primary">Confirm</button> </div> </div> </div>
    ...
    function initializeEditor() {
        ...
        // Force execCommand to use CSS (spans) instead of deprecated HTML tags (font).
        // This is the first step to modernizing without breaking core functionality.
        ...
        // --- MODERN API REPLACEMENT ---
        // Replaced `execCommand` for paste with the modern Selection and Range APIs
        ...
        // Note: `execCommand` for `undo` and `redo` is kept as there is no native browser
        // replacement for the editor's history stack without a custom implementation.
        ...
        document.querySelectorAll('.richtext-btn[data-command]').forEach(element => {
        ...
        // --- MODERN API REPLACEMENT ---
        // Reverted to the standard `execCommand` API for block formatting. It's the browser's
        // native implementation for this and is better equipped to handle complex selections
        // across multiple paragraphs than a custom implementation.
        ...
        function toggleList(listType) {
            editor.focus();
            const command = listType === 'ol' ? 'insertOrderedList' : 'insertUnorderedList';
        ...
        const genericModal = { overlay: document.getElementById('modalOverlay'), title: document.getElementById('modalTitle'), text: document.getElementById('modalText'), input: document.getElementById('modalInput'), cancel: document.getElementById('modalCancel'), confirm: document.getElementById('modalConfirm'), onConfirmCallback: null, show({ title, text, showInput = false, confirmText = 'OK' }) { this.title.textContent = title; this.text.textContent = text; this.input.style.display = showInput ? 'block' : 'none'; this.input.value = ''; this.confirm.textContent = confirmText; this.overlay.style.display = 'flex'; this.input.focus(); return new Promise((resolve) => { this.onConfirmCallback = resolve; }); }, hide() { this.overlay.style.display = 'none'; if (this.onConfirmCallback) { this.onConfirmCallback(null); this.onConfirmCallback = null; } } };
        genericModal.cancel.addEventListener('click', () => genericModal.hide());
        genericModal.overlay.addEventListener('click', (e) => { if(e.target === genericModal.overlay) genericModal.hide(); });
        genericModal.confirm.addEventListener('click', () => { const value = genericModal.input.style.display !== 'none' ? genericModal.input.value : true; if (genericModal.onConfirmCallback) { genericModal.onConfirmCallback(value); } genericModal.onConfirmCallback = null; genericModal.overlay.style.display = 'none'; });
        ...
        // --- MODERN API REPLACEMENT ---
        // This function now uses a simplified and more robust method to apply font styles.
        // It still uses a marker-based approach for reliability across complex selections,
        // but the cleanup logic is streamlined to prevent errors.
        function applyInlineStyle(property, value) {
            ...
            // Handle typing at a collapsed cursor by creating a styled span.
            if (range.collapsed) {
            ...
            // For selected text, use a robust marker-based approach.
            // execCommand handles complex selections, and we clean up the resulting deprecated tags.
            document.execCommand('fontSize', false, '7');
            const elements = editor.querySelectorAll("font[size='7']");

            elements.forEach(fontEl => {
                const newSpan = document.createElement('span');
                newSpan.style[property] = value;
        ...
        // Handles color changes, where execCommand is generally reliable.
        const applyColorToSelection = (property, value) => {
            editor.focus();
            const command = property === 'color' ? 'foreColor' : 'hiliteColor';
            document.execCommand(command, false, value);
        };


        fontNameSelect.addEventListener('change', (e) => applyInlineStyle('fontFamily', e.target.value));
        fontSizeSelect.addEventListener('change', (e) => applyInlineStyle('fontSize', e.target.value));
        formatBlockSelect.addEventListener('change', (e) => applyBlockFormat(e.target.value));
        
        foreColorPicker.addEventListener('input', (e) => {
            applyColorToSelection('color', e.target.value);
            document.getElementById('foreColorIndicator').style.backgroundColor = e.target.value;
        });
        backColorPicker.addEventListener('input', (e) => {
            applyColorToSelection('backgroundColor', e.target.value);
            document.getElementById('backColorIndicator').style.backgroundColor = e.target.value;
        });
